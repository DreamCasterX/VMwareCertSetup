...
class SUTConfigurator(BaseConfigurator):
    def __init__(self):
        ...
        self.default_submask = "255.255.255.0"
        self.default_gateway = "192.168.1.1"
        self.default_dns = "192.168.1.1"
...
"""
 ...
 ___________________________________________________________________{Style.RESET_ALL}
    """
        ...
        # 獲取內部 IP
        while True:
            internal_ip = input("Enter VIVa local IP address (192.168.x.x): ").strip()
...
class AgentConfigurator(BaseConfigurator):
 ...
 _______________________________________________________________________{Style.RESET_ALL}
    """
        ...
        ssh_client = None
        while True:
            internal_ip = input("Enter Agent local IP address (192.168.x.x): ").strip()
            if self.validate_ip(internal_ip):
...
class PciPassthruConfigurator(BaseConfigurator):
    def __init__(self):
        ...
        self.password = "Passw0rd!"   # ESXi 密碼
...
class OVFManager(BaseConfigurator):
    ...
    def get_ovf_networks(self, ovf_file):
        """用 XML 解析 OVF network name，並同時解析 VM 名稱存到 self.last_vm_name"""
        try:
            ovf_content = None
    ...
    def get_target_networks(self, si):
        """查詢 ESXi host 上所有可用的 target networks"""
        try:
            content = si.RetrieveContent()
            ...
            # 顯示ESXi上面可用的網路
            # print("Networks found on ESXi host:", networks)  
            container.Destroy()
            return networks
        except Exception as e:
            print(f"{Fore.RED}Error retrieving target networks: {e}{Style.RESET_ALL}")
            return []
        ...
        try:
            while True:
                ...
                if host:
                    ...
                    # 保留彈性讓user自行輸入密碼或直接用預設 (若不想讓user設定可直接刪除下面這行)
                    self.password = self.get_valid_input(f"Enter ESXi password <press Enter to accept default {Fore.CYAN}Passw0rd!{Style.RESET_ALL}>: ", default="Passw0rd!").strip()
...
class VCConfigurator(BaseConfigurator):
    def __init__(self):
        ...
        self.default_TC_host = "10.240.226.13"
        self.default_TC_password = "Passw0rd!"   
        self.default_VC_prefix = "24"
        self.default_VC_gateway = "192.168.1.1"
 ...
 _________________________________________________________________________________________{Style.RESET_ALL}
    """
        ...
        suggested_system_name = f"vc{network_config['ip'].split('.')[-1]}.mty.com"
        ...
        try:
            ...
            if deployment_success:
                ...
                webbrowser.get("windows-default").open_new(url_vcsa)
...
class DNSConfigurator(BaseConfigurator):
    def __init__(self):
        super().__init__()
        self.default_dns_server = "192.168.1.10"  # 預設 DNS 伺服器 IP
        self.default_zone = "mty.com"         # 預設 DNS 區域
        self.username = "Administrator"          # 預設 DNS 伺服器使用者名稱
        self.password = "Admin!23"               # 預設 DNS 伺服器密碼

    def show_dns_records(self, ssh_client, zone: str) -> list:
        """顯示指定區域的 DNS A 記錄並返回列表"""
        try:
            cmd = f'powershell -Command "Get-DnsServerResourceRecord -ZoneName \"{zone}\" | Where-Object {{ $_.RecordType -eq \"A\" }} | Select-Object HostName, @{{Name=\"RecordData\";Expression={{ $_.RecordData.IPv4Address }}}} | ConvertTo-Json -Compress"'
            ...
            if output:
                ...
                if records:
                    ...
                    for i, record in enumerate(records, 1):
                        ...
                        ip_address = record.get('RecordData', 'N/A')
                        print(f"{hostname:<30}{ip_address:<15}")
                else:
            ...
            print(f"{Fore.RED}Error showing DNS records: {e}{Style.RESET_ALL}")
            print(f"{'='*30}\n")
            return []
    ...
    def delete_dns_host(self, ssh_client, zone: str, hostname: str) -> bool:
        ...
        try:
            # 使用 dnscmd 刪除 A 記錄
            cmd = f'dnscmd . /recorddelete {zone} {hostname} A'
            ...
            if exit_status == 0:
                ...
                error = stderr.read().decode()
                print(f"{Fore.RED}Failed to delete DNS record: {error}{Style.RESET_ALL}")
                return False
 ...
 ___________________________________________________________________{Style.RESET_ALL}
    """
        ...
        try:
            ...
            print("  1) Add DNS host record")
            print("  2) Delete DNS host record")
            while True:
                mgmt_choice = input("Enter your choice (1-2): ").strip()
                if mgmt_choice in ['1', '2']:
                    break
                print(f"{Fore.YELLOW}Please enter '1' or '2'{Style.RESET_ALL}")

            if mgmt_choice == '1':
                # 添加 DNS 記錄的流程 (原有的)
                # 顯示現有的 DNS 記錄
                self.show_dns_records(ssh_client, zone)
                ...
                while True:
                    ...
                    if self.add_dns_host(ssh_client, zone, hostname, ip_address):
                        print(f"{Fore.GREEN}DNS record has been added successfully{Style.RESET_ALL}")
                        print(f"\nRecord details:")
                ...
                # 刪除 DNS 記錄的流程
                while True:
                    records_to_delete = self.show_dns_records(ssh_client, zone)
                    if not records_to_delete:
                        print(f"{Fore.YELLOW}No records to delete or failed to retrieve records.{Style.RESET_ALL}")
                        break # 如果沒有記錄或獲取失敗，退出刪除流程

                    print("Select a record to delete:")
                    for i, record in enumerate(records_to_delete, 1):
                         print(f"  {i}) {record.get('HostName', 'N/A'):<30}{record.get('RecordData', 'N/A'):<15}")
                    print(f"  0) Cancel")

                    while True:
                        try:
                            delete_choice = input("Enter the number of the record to delete (or 0 to cancel): ").strip()
                            delete_index = int(delete_choice) - 1
                            if delete_index == -1: # User entered 0 to cancel
                                break
                            if 0 <= delete_index < len(records_to_delete):
                                ...
                                if hostname_to_delete:
                                     if self.delete_dns_host(ssh_client, zone, hostname_to_delete):
                                         ...
                                         print("\nUpdated DNS records:")
                                         self.show_dns_records(ssh_client, zone)
                                     break # 刪除成功或失敗都退出當前選擇迴圈
                                else:
                                     print(f"{Fore.RED}Could not get hostname for selected record.{Style.RESET_ALL}")
                                     break
                            else:
                                print(f"{Fore.YELLOW}Please enter a valid number or 0.{Style.RESET_ALL}")
                        except ValueError:
                            print(f"{Fore.YELLOW}Please enter a valid number.{Style.RESET_ALL}")

                    if delete_index == -1: # User canceled deletion
                         break
...
7) Add PCI Passthrough VM Options (NVIDIA GPU)
8) Copy agent log
9) Exit
