...
from main import SUTConfigurator, VIVaConfigurator, AgentConfigurator, VCConfigurator, OVFManager, DNSConfigurator, ResultLogCopier, PciPassthruConfigurator
import pandas as pd
import os
import json
import subprocess
...
# Monkey patch: add get_available_networks to OVFManager and VCConfigurator if they don't have it
def _get_available_networks(self, si):
    try:
        content = si.RetrieveContent()
        container = content.viewManager.CreateContainerView(content.rootFolder, [vim.Network], True)
        networks = []
...
if not hasattr(OVFManager, 'get_target_networks'):
     OVFManager.get_target_networks = _get_available_networks

if not hasattr(VCConfigurator, 'get_available_networks'):
    VCConfigurator.get_available_networks = _get_available_networks

# Helper for resetting Agent Config flow
RESET_AGENT_KEYS = [
    'agent_step', 'agent_form_data', 'agent_ssh', 'agent_success_msgs', 
    'agent_uploaded', 'agent_external_dns', 'agent_url', 'agent_overwrite_confirm', 
    'agent_pending_file', 'agent_external_ip'
]
...
# Helper for resetting vCenter Deploy flow
RESET_VC_KEYS = ['vc_step', 'vc_form_data', 'vc_iso_path', 'vc_si', 'vc_config_json_path']
def vc_reset_button():
    if st.button("Reset/Restart"):
        for k in RESET_VC_KEYS:
            if k in st.session_state:
                del st.session_state[k]
        st.rerun()

def get_sysinfo(ssh):
    """Gathers system information from ESXi host via SSH and returns it as a dictionary."""
    info = {}
    sections = [
        ("System Information", {
    ...
    for section_name, commands in sections:
        ...
        for label, command in commands.items():
            ...
            info[section_name][label] = result
    return info
...
with col_content:
    if choice == "Config SUT":
        ...
        **Prerequisites**
        ...
        if st.session_state['sut_step'] == 0:
            ...
            with form_col:
                with st.form("sut_form"):
                    ...
                    root_password = st.text_input("Enter root password", value="Admin!23", type="password")
                    static_ip = st.text_input("New static IP address")
                    subnet = st.text_input("Subnet Mask", value="255.255.252.0")
                    gateway = st.text_input("Default Gateway", value="192.168.4.1")
                    dns = st.text_input("Primary DNS", value="192.168.4.1")
        ...
        def viva_reset_button():
            if st.button("Reset/Restart"):
                for k in ['viva_step','viva_form_data','viva_ssh','viva_success_msgs','viva_continue_clicked']:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()

        from main_BDC import VIVaConfigurator
        c = VIVaConfigurator()
            ...
            **Prerequisites**
            ...
            with form_col:
                with st.form("viva_form"):
                    viva_ip = st.text_input("Enter VIVa IP (192.168.x.x)")
                    external_dns = st.text_input("External DNS IP", value="10.241.96.14")
                    submitted = st.form_submit_button("Start Configuration", type="primary")
            if submitted:
                if not c.validate_ip(viva_ip):
                    st.error("❌ Invalid VIVa IP format")
                    st.stop()
                if not c.validate_ip(external_dns):
                    st.error("❌ Invalid external DNS format")
                    st.stop()
                ssh = c.ssh_connect(viva_ip, c.username, c.password)
                if not ssh:
                    st.error("❌ SSH connection to VIVa failed. Please check IP/password/SSH status.")
                    st.stop()
                st.session_state['viva_form_data'] = {'viva_ip': viva_ip, 'external_dns': external_dns}
            ...
            st.info("Switching to external network and configuring...")
            data = st.session_state['viva_form_data']
            ssh = st.session_state['viva_ssh']
            # 用同一個 IP，gateway/dns 由 VIVaConfigurator 決定
            success, new_ssh = c.configure_external_network_config(ssh, data['viva_ip'], data['external_dns'])
            if not success or not new_ssh:
                st.error("❌ Failed to configure external network.")
                viva_reset_button()
                st.stop()
            st.session_state['viva_ssh'] = new_ssh
            st.session_state['viva_success_msgs'].append("External network configuration successful.")
            st.session_state['viva_step'] = 3
            st.rerun()

        elif st.session_state['viva_step'] == 3:
            st.info("Checking Internet connectivity via external network...")
            ssh = st.session_state['viva_ssh']
            def check_internet_with_timeout(ssh, timeout=15):
                def inner():
                    return c.check_internet(ssh)
            ...
            if result == "timeout":
                ...
                st.error("❌ Internet connectivity check failed (external network).")
                viva_reset_button()
                st.stop()
            st.session_state['viva_success_msgs'].append("Internet connectivity check successful (external network).")
            ...
            with st.spinner("🔄 Refreshing VIVa service..."):
                try:
                    ...
                    st.error(f"❌ Error refreshing VIVa service: {e}")
                    viva_reset_button()
                    st.stop()
            ...
            st.info("Switching back to internal network...")
            data = st.session_state['viva_form_data']
            ssh = st.session_state['viva_ssh']
            # 用同一個 IP，gateway/dns 由 VIVaConfigurator 決定
            success, new_ssh = c.configure_internal_network_config(ssh, data['viva_ip'])
            if not success or not new_ssh:
                st.error("❌ Failed to configure internal network.")
                viva_reset_button()
                st.stop()
            st.session_state['viva_ssh'] = new_ssh
            st.session_state['viva_success_msgs'].append("Internal network configuration successful.")
            st.session_state['viva_step'] = 6
            st.rerun()

        elif st.session_state['viva_step'] == 6:
            st.info("Updating jump server hosts file...")
            data = st.session_state['viva_form_data']
            ssh = st.session_state['viva_ssh']
            if not c.update_jump_server_hosts_file(data['viva_ip']):
                st.error("❌ Failed to update jump server hosts file.")
                viva_reset_button()
                st.stop()
            st.session_state['viva_success_msgs'].append("Jump server hosts file configuration successful.")
            st.session_state['viva_step'] = 7
            st.rerun()

        elif st.session_state['viva_step'] == 7:
        ...
        def agent_reset_button():
            if st.button("Reset/Restart"):
                for k in ['agent_step','agent_form_data','agent_ssh','agent_success_msgs','agent_uploaded','agent_url']:
                    if k in st.session_state:
                        del st.session_state[k]
                st.rerun()

        from main_BDC import AgentConfigurator
        c = AgentConfigurator()
        import concurrent.futures
            ...
            **Prerequisites**
            ...
            with form_col:
                with st.form("agent_form"):
                    agent_ip = st.text_input("Enter Agent IP (192.168.x.x)")
                    external_dns = st.text_input("External DNS IP", value="10.241.96.14")
                    submitted = st.form_submit_button("Start Configuration", type="primary")
            if submitted:
                if not c.validate_ip(agent_ip):
                    st.error("❌ Invalid Agent IP format")
                    st.stop()
                if not c.validate_ip(external_dns):
                    st.error("❌ Invalid external DNS format")
                    st.stop()
                ...
                if not ssh:
                    st.error("❌ SSH connection to Agent failed. Please check IP/password/SSH status.")
                    st.stop()
                st.session_state['agent_form_data'] = {'agent_ip': agent_ip, 'external_dns': external_dns}
                st.session_state['agent_ssh'] = ssh
                st.session_state['agent_uploaded'] = False
                st.session_state['agent_step'] = 1
                st.rerun()

        # 步驟 1: 上傳 runlist.json
        ...
        # 步驟 2: 切換到外部網路
            ...
            st.info("Switching to external network and configuring...")
            data = st.session_state['agent_form_data']
            ssh = st.session_state['agent_ssh']
            success, new_ssh = c.configure_external_network_config(ssh, data['agent_ip'], data['external_dns'])
            if not success or not new_ssh:
                st.error("❌ Failed to configure external network.")
                agent_reset_button()
                st.stop()
            st.session_state['agent_ssh'] = new_ssh
            st.session_state['agent_success_msgs'].append("External network configuration successful.")
        ...
        # 步驟 3: 檢查 Internet
            ...
            if result == "timeout":
                ...
                agent_reset_button()
                st.stop()
            st.session_state['agent_success_msgs'].append("Internet connectivity check successful (external network).")
            st.session_state['agent_step'] = 4
            st.rerun()

        # 步驟 4: 執行 AgentLauncher
                ...
                try:
                    ...
                    for line in iter(stdout.readline, ''):
                        if line:
                            ...
                            # 進度條解析（仿 app_MTY.py）
                            if 'Pulling fs layer' in cleaned_line:
                                total_layers += 1
                            elif 'Pull complete' in cleaned_line:
                                completed_layers += 1
                                if total_layers > 0:
                                    percent = int(completed_layers / total_layers * 100)
                                    progress_bar.progress(percent)
                                    progress_text.text(f"Pulling layers: {completed_layers}/{total_layers}")
                    exit_status = stdout.channel.recv_exit_status()
                    if exit_status == 0:
                        progress_bar.progress(100)
                        progress_text.text("Deployment successful!")
                        st.success("🎉 Agent deployment successful!")
                        st.session_state['agent_success_msgs'].append("Agent deployment successful!")
                        agent_ip = st.session_state['agent_form_data']['agent_ip']
                        st.session_state['agent_url'] = f"https://{agent_ip}/agent-ui"
                        st.session_state['agent_step'] = 5
                        st.rerun()
                    else:
        ...
        # 步驟 5: 切回內部網路
        elif st.session_state['agent_step'] == 5:
            st.info("Switching back to internal network...")
            data = st.session_state['agent_form_data']
            ssh = st.session_state['agent_ssh']
            success, new_ssh = c.configure_internal_network_config(ssh, data['agent_ip'])
            if not success or not new_ssh:
                st.error("❌ Failed to configure internal network.")
                agent_reset_button()
                st.stop()
            st.session_state['agent_ssh'] = new_ssh
            st.session_state['agent_success_msgs'].append("Internal network configuration successful.")
            st.session_state['agent_step'] = 6
            st.rerun()

        # 步驟 6: 完成
        elif st.session_state['agent_step'] == 6:
        ...
        if st.session_state.vc_step == 0:
            ...
            **Prerequisites**
            ...
            with st.form("vc_form"):
                ...
                esxi_host = st.text_input("TC IP address")
                esxi_username = st.text_input("TC username", value=c.default_TC_username)
                esxi_password = st.text_input("TC password", type="password")
                ...
                vc_ip = st.text_input("vCenter IP address", placeholder="e.g., 192.168.4.99")
                vc_system_name = st.text_input("vCenter system name (FQDN)", placeholder="e.g., vc99.lenovo.com")
                ...
                with st.spinner("Starting vCenter deployment... This will take a long time."):
                    ...
                    for line in iter(process.stdout.readline, ''):
                        if line:
                            ...
                            elif (('OVF Tool: Completed successfully' in cleaned_line or 'Transfer Completed' in cleaned_line)):
                                progress_bar.progress(50)
                                progress_text.text("Appliance deployed. Waiting for vCenter to initialize...")
                            elif 'OVF Tool' in cleaned_line and 'Disk progress:' in cleaned_line:
                                try:
                                    percent_str = cleaned_line.split('Disk progress:')[1].strip().replace('%', '')
                                    ovf_percent = int(percent_str)
                                    overall_percent = int(ovf_percent * 0.5)
                                    progress_bar.progress(overall_percent)
        ...
        if ovf_mode == "Deploy OVF":
            ...
            if st.session_state['ovf_step'] == 0:
                with st.form("ovf_deploy_form"):
                    ...
                    esxi_password = st.text_input("ESXi password", type="password", value="Admin!23")
        ...
        **Prerequisites**
        ...
        from main import PciPassthruConfigurator
        ...
        with form_col:
            if st.session_state['pci_step'] == 0:
                ...
                log_lines = []
                try:
                    # 直接複製 main.py 的 add_vm_options 主要邏輯
                    content = si.RetrieveContent()
                    vm = None
                    container = content.viewManager.CreateContainerView(content.rootFolder, [vim.VirtualMachine], True)
...
if __name__ == "__main__":
    # --- Add Streamlit-specific wrappers to classes that need them ---
    def deploy_ovf_streamlit(self, host, password, ovf_file, vm_name, datastore, network_mapping):
        """Streamlit wrapper for deploy_ovf to construct and run the command."""
        net_args = [f'--net:{ovf_net}={target_net}' for ovf_net, target_net in network_mapping.items()]
        cmd = [
            self.ovf_tool_path, '--noSSLVerify', '--acceptAllEulas',
            '--X:logLevel=verbose', f'--datastore={datastore}', f'--name={vm_name}',
        ] + net_args + [
            ovf_file, f'vi://{self.user}:{password}@{host}'
        ]
        try:
            # Using Popen to allow Streamlit to continue rendering
            process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            # This is not a blocking call in the UI thread. For real-time output, more complex handling is needed.
            # Here we just wait for completion in this simple implementation.
            ...
            print(f"Error in deploy_ovf_streamlit: {e}")
            return False

    def get_vm_by_name(self, si, vm_name):
        """Utility to get a VM object by its name."""
        content = si.RetrieveContent()
        container = content.viewManager.CreateContainerView(content.rootFolder, [vim.VirtualMachine], True)
        vm = next((vm for vm in container.view if vm.name == vm_name), None)
        container.Destroy()
        return vm

    def export_ovf_streamlit(self, host, password, vm_name, output_file, si):
        """Streamlit wrapper for export_ovf."""
        vm = self.get_vm_by_name(si, vm_name)
        if not vm: return False
        if vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOn:
            task = vm.PowerOffVM_Task() # Power off before export
            while task.info.state not in [vim.TaskInfo.State.success, vim.TaskInfo.State.error]: time.sleep(1)
        
        cmd = [
            self.ovf_tool_path, '--noSSLVerify', '--acceptAllEulas', '--overwrite',
            f'vi://{self.user}:{password}@{host}/{vm_name}',
        ...
        try:
            ...
            print(f"Error in export_ovf_streamlit: {e}")
            return False

    def delete_vm_streamlit(self, si, vm_name):
        """Streamlit wrapper for delete_vm."""
        try:
            vm = self.get_vm_by_name(si, vm_name)
            if not vm: return False
            if vm.runtime.powerState == vim.VirtualMachinePowerState.poweredOn:
            ...
            print(f"Error in delete_vm_streamlit: {e}")
            return False

    OVFManager.deploy_ovf_streamlit = deploy_ovf_streamlit
    OVFManager.get_vm_by_name = get_vm_by_name
    OVFManager.export_ovf_streamlit = export_ovf_streamlit
    OVFManager.delete_vm_streamlit = delete_vm_streamlit
